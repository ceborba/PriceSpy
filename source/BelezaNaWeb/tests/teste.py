...
def salvar_dados_no_banco(dados):
    ...
    for produto in dados:
        ...
        if resultado:
            # Produto já existe, verificar se o preço mudou
            ...
            if preco_antigo != produto['price']:
                ...
                novos_produtos.append((id_produto, produto['name'], preco_antigo, produto['price']))
        else:
            # Novo produto, inserir no banco de dados
            cursor.execute(''' 
                INSERT INTO produtosBW (url_key, name, price, url)
                VALUES (?, ?, ?, ?)
            ''', (produto['url_key'], produto['name'], produto['price'], produto['url']))
            id_produto = cursor.lastrowid
            novos_produtos.append((id_produto, produto['name'], produto['price'], produto['price']))
