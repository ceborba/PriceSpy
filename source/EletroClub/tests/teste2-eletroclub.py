        ...
        if result:
            ...
            if mudancas:
                ...
                if preco_atual < preco_antigo:
                    ...
                    self.enviar_webhook_discord(nome, preco_antigo, preco_atual, link_produto, desconto_percentual)
            return True
        else:
            cursor.execute('''
            ...
            link_produto = f"https://www.eletroclub.com.br{link}"
            self.enviar_webhook_discord(nome, preco_atual, preco_atual, link_produto, 0, novo_produto=True)
        return False


    def enviar_webhook_discord(self, nome_produto, preco_antigo, preco_novo, link_produto, desconto_percentual, novo_produto=False):
        webhooks = {
            ...
            "novos": "https://discord.com/api/webhooks/1332241750881992784/BbJC4Fpd-z-VpA0awNZAvoh6MrYuqnxOHtx4cv5YjXrtXFe31K8s6rVtzETVhh2igpa-"
        }
                ...
                if data and 'data' in data and 'productSearch' in data['data'] and 'products' in data['data']['productSearch']:
                    ...
                    for product in products:
                        try:
                            ...
                            link = product['link']
                            try:
                                preco_base_low = product['priceRange']['sellingPrice']['lowPrice']
                                preco_base_high = product['priceRange']['sellingPrice']['highPrice']
                                
                                if preco_base_low != preco_base_high:
                                    logging.warning(f"Inconsistência de preço para o produto {produto_id}: lowPrice={preco_base_low}, highPrice={preco_base_high}")
                                    preco_base = preco_base_high  # Usando o preço mais alto como base
                                else:
                                    preco_base = preco_base_high  # Usando o preço mais alto mesmo quando são iguais

                                preco_list_low = product['priceRange']['listPrice']['lowPrice']
                                preco_list_high = product['priceRange']['listPrice']['highPrice']

                                if preco_list_low != preco_list_high:
                                    logging.warning(f"Inconsistência no preço de lista para o produto {produto_id}: lowPrice={preco_list_low}, highPrice={preco_list_high}")
                                    preco_desc = max(preco_list_low, preco_list_high) - preco_base
                                else:
                                    preco_desc = preco_list_high - preco_base if preco_list_high > preco_base else None
                            except KeyError as e:
                                logging.error(f"Erro ao acessar dados de preço para o produto {produto_id}: {e}")
                                continue
